...
Let \\( x \\) be a time series of length \\( n \\), then representation of \\( x \\) is a model $\hat{x}$ with reduced dimensionality \\( p \\) \\( (p < n) \\) such that $\hat{x}$ approximates closely \\( x \\).
...
In **data dictated** approaches, the compression ratio is defined automatically based on raw time series such as Clipped.
...
The **data dictated** is the less known type of representation and most famous method of this type is clipping (bit-level) representation.
...
Also additional useful functions are implemented as:
...
data_plot <- data.frame(Value = c(data_dwt, data_dft, data_dct, data_paa),
                        ...
                        Method = factor(rep(c("DWT", "DFT", "DCT", "PAA"), each = length(data_dwt))))
...
For seasonal time series as electricity load data, the model-based representations are highly recommended. By model-based representation, we can extract a daily profile of some consumer. We can do it by simple average (or median) daily profile or by extraction of **seasonal regression coefficients**.
...
data_plot <- data.frame(Value = c(data_lm, data_l1, data_seas_prof, data_exp, data_gam),
                        ...
                        Method = c(rep(c("LM", "L1", "Median seas. prof.", "Exp. smooth."), each = 48), rep("GAM", 47)))
...
data_plot <- data.frame(Value = c(data_ts, data_pip$points, data_pla$points),
                        ...
                        Method = c(rep("Original", length(data_ts)), rep(c("PIP", "PLA"), each = length(data_pla$places))))
...
We can see some significant differences among these two methods.
...
Both creates bit-level (binary) representation from original time series and computes run lengths of values by RLE (Run Length Encoding). Then interpretable features are extracted from run lengths. I will now describe first of the mentioned methods - **FeaClip **. Clipping representation is created very easily - if a value of a time series is greater then its average value then value is transformed to 1 and otherwise to 0. It can be defined formally as follows:
...
The second data dictated method is **FeaTrend**. It extracts features from "trending" (again bit-level) representation. The trending representation is defined as follows:
...
Before the computation of trending representation, a time series is smoothed (denoised) by simple moving average method (`repr_sma`) in order to have more compact run lengths. Let's demonstrate this factor in our example case, so we will use `trending` and RLE function on original and also for smoothed time series:
```{r}
...
So, maximal run length of ones is 15 and maximal run length of zeros is 30, as expected, the number of zeros is much more than ones because of decreasing character of the time series.
