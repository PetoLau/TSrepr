...
In this vignette (tutorial), I will show you one use case to use effectively time series representations. This use case is **clustering** of time series and it will be clustering of consumers of electricity load.

## Clustering of time series representations

By clustering of consumers of electricity load, we can extract typical load **profiles**, improve the accuracy of consequent electricity consumption forecasting, detect anomalies or monitor whole smart grid (grid of consumers) (@Lau1, @Lau2). I will show you the first use case, so extraction of typical electricity load profiles by [**K-means**](https://en.wikipedia.org/wiki/K-means_clustering) clustering method.

Firstly, let's load needed packages, data (`elec_load`) and return the dimensions of the dataset.
```{r, message=F}
library(TSrepr)
library(ggplot2)
library(data.table)
...
For double seasonal time series of electricity consumption (daily and weekly seasonality), the model-based representation approaches seem to have best ability to extract typical profiles. But, I will use some other representations too.
...
That is more nice result, isn't it? Extracted consumption profiles are smoother than in the case of average seasonal profiles. The blue dashed line borders daily and weekly seasonal coefficients.
