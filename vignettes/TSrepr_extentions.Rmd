...
I will show you now, how to apply it on whole dataset (by function `repr_matrix`), **cluster** final representations and then interpret results. Before applying clustering on electricity consumption data, firstly normalisation is needed. We can use classical z-score (`norm_z`) or min-max (`norm_min_max`) normalisation methods for every consumers time series. However, there is possibility to use directly, in function `repr_matrix`, arbitrary defined normalisation function. Let's use for example simple self-defined max normalisation.
```{r}
norm_max <- function(x) {
  return(x/max(x))
}
...
There are three dominant clusters (n. 1, 2 and 3). Time series in clusters n. 4 and 5 are irregular against other time series, so there were assigned to own clusters.
